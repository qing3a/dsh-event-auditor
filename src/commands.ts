...
export interface CommandsRegistrar {
  register(definition: {
    name: string
    description: string
    handler(invocation: { commandId: string; rawInput: string }): { kind: string; text?: string } | Promise<{ kind: string; text?: string }>
  }): () => void
}

/** 渲染审计摘要为给人看的文本。 */
function renderSummary(auditor: AuditorService): string {
...
export function registerAuditCommand(commands: CommandsRegistrar, auditor: AuditorService): () => void {
