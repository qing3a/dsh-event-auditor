...
- 已实证：waterfall 的运行时捕获——用仓库自带 mock-llm + headless profile 完成验证（74 事件 / 12 waterfall，agent 完整跑完工具循环），详见系列第三篇与 [GitHub Discussion 462](https://github.com/deepseek-ai/deepseek-harness/discussions/462)
...
下一篇预告：**第三篇——无 API Key 验证 DSH 插件：mock-llm 实战与 waterfall 捕获实证**（零成本复现完整 agent 循环，实证 74 事件 / 12 waterfall）。
